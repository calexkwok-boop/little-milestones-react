...
function GrowthChart({ measurements, refTable, color }) {
  ...
  const maxAgeMo = Math.min(60, Math.max(
    hasMeasurements ? Math.max(...measurements.map(m => m.age)) + 4 : 24,
    refTable ? refTable[refTable.length - 1][0] : 24,
  ));
