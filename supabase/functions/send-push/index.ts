...
// client-supplied title/body text.
function buildPayload(kind: string, ctx: Record<string, unknown>) {
  const fromName = (ctx.fromName as string) || 'Someone';
  switch (kind) {
    case 'like':
      return { title: 'New reaction', body: `${fromName} liked your ${ctx.kidNames ? `letter to ${ctx.kidNames}` : 'entry'}`, url: `/?open=${ctx.entryId}`, tag: `like-${ctx.entryId}` };
    case 'comment':
      return { title: 'New comment', body: `${fromName}: "${truncate(String(ctx.commentPreview || ''), 120)}"`, url: `/?open=${ctx.entryId}`, tag: `comment-${ctx.entryId}` };
    case 'reply':
      return { title: 'New reply', body: `${fromName} replied to your comment`, url: `/?open=${ctx.entryId}`, tag: `reply-${ctx.entryId}` };
    case 'friend_request':
      return { title: 'New friend request', body: `${fromName} wants to connect on Patina`, url: '/', tag: 'friend-request' };
    default:
      return null;
  }
}
